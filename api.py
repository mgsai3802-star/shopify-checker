...
# ==========================================
# AUTO FREE PROXY SCRAPER (အလိုအလျောက် Free Proxy ရှာဖွေသည့်စနစ်)
# ==========================================
FREE_PROXIES = []
LAST_PROXY_FETCH = 0

async def update_free_proxies():
    global FREE_PROXIES, LAST_PROXY_FETCH
    current_time = time.time()
    # ၁၀ မိနစ်ပြည့်တိုင်း Proxy အသစ်များကို ProxyScrape မှ Auto ဆွဲယူမည်
    if not FREE_PROXIES or (current_time - LAST_PROXY_FETCH > 600):
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, timeout=10) as resp:
                    if resp.status == 200:
                        ...
                        FREE_PROXIES = [f"http://{line.strip()}" for line in text.split('\n') if line.strip()]
                        LAST_PROXY_FETCH = current_time
                        print(f"[+] Successfully fetched {len(FREE_PROXIES)} free proxies.")
        except Exception:
            pass
...
# =========================================================
# ** ကျန်ရစ်ခဲ့သော extract_clean_response ကို ပြန်ထည့်သွင်းခြင်း **
# =========================================================
...
def shopify_checker():
    try:
        ...
        try:
            # တကယ်လို့ ကိုယ်ပိုင် Proxy မထည့်ထားရင် Free Proxy ကို Auto ဆွဲယူသုံးစွဲမည်
            if not proxy_str:
                loop.run_until_complete(update_free_proxies())
